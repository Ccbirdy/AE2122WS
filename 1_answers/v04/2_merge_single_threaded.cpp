...
#include <random>
using namespace std;
// g++  -Ofast -std=c++11 -march=native -fopenmp 2_merge_single_threaded.cpp -o ms
...
inline void merge_dac(_Type *t1, _Type *t2, int64_t p1, int64_t r1, int64_t p2,
  ...
  if (length1 == 0)
    return;
  if ((length1 + length2) <= 8192 * 8) { // small array wonc recrusion
    ...
    // if not say the parameter will be first private
#pragma omp task
    merge_dac(t1, t2, p1, q1 - 1, p2, q2 - 1, a, p3, __comp);
#pragma omp task // or without this one, not so much difference in speed
