...
#include <omp.h>
#include <vector>
// g++  -Ofast -std=c++11 -march=native -fopenmp 3_mandelbrot_tuned.cpp -o mb
...
#pragma omp for schedule(dynamic) ordered // we could also use schedule(static, 1)  
      // here should be ordered
      for (int i = 0; i < height; i++) {
        buffer.clear(); // clear content of buffer // not enough for loops to collapse  // why not private(buffer)
